...
export async function extractBillAmount(formData: FormData): Promise<{ success: boolean, amount?: number, error?: string }> {
    try {
        ...
        if (!apiKey) {
            return { success: false, error: "V7-ACTION Greška: GEMINI_API_KEY nije postavljen u Vercel Environment Variables!" };
        }
...
3. Vrati SAMO broj (npr. 319.64). Bez teksta i valuta.`;
        ...
        if (!response.ok) {
            throw new Error(`Google V7-ACTION Error ${response.status}: ${data?.error?.message || "Provjerite ključ"}`);
        }
        ...
        if (!isNaN(extractedNumber)) {
            ...
            return { success: false, error: "V7-ACTION: AI nije pronašao broj. Vratio je tekst: " + responseText.substring(0, 50) };
        }

    } catch (error: any) {
        console.error("AI OCR Greška na Vercelu:", error);
        return { success: false, error: "V7-ACTION Greška: " + (error.message || "Provjerite API ključ") };
    }
}
